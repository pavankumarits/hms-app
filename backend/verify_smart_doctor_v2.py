...
def test_symptom_checker():
    ...
    url = f"{BASE_URL}/predict-diagnosis"
    data = {
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            if result and (result[0]['name'] == 'Viral Fever' or result[0]['name'] == 'Viral Upper Respiratory Infection'):
                print("[OK] Symptom Checker Passed!")
            else:
                print("[FAIL] Symptom Result Mismatch")
    except Exception as e:
        print(f"[FAIL] Symptom Checker Failed: {e}")
